 ...
 */
double diff(const std::function<double(double)> &f, double x) {
    const double h = 1e-6;
 ...
 * 
 * @return The gradient of f at x as a vector of size X.
 */
template <int X>
EVec<X> gradient(const std::function<double(const EVec<X> &x)> &f, const EVec<X> &x) {
 ...
 * 
 * @return The directional derivative of f at x in the direction of v.
 */
template <int X>
double directional_derivative(const std::function<double(const EVec<X> &x)> &f, const EVec<X> &x, const EVec<X> &v) {
    return (v * gradient(f, x)) ;
}
 ...
 */
template <int X>
EMat<X, X> jacobian_wrt_x(const std::function<EVec<X>(const EVec<X> &)> &f, const EVec<X> &x) {
...
EMat<X, X> jacobian_wrt_x(const std::function<EVec<X>(const EVec<X> &x, const EVec<U> &u)> &f, const EVec<X> &x, const EVec<U> &u) {
    EMat<X, X> jac;
    const double h = 1e-6;
    for (int i = 0; i < X; i++) {
        EVec<X> x_plus = x;
 ...
 */
template <int X, int U>
EMat<X, U> jacobian_wrt_u(const std::function<EVec<X>(const EVec<X> &x, const EVec<U> &u)> &f, const EVec<X> &x, const EVec<U> &u) {
